...
export function Overview({ logs, onSelect }: OverviewProps) {
  ...
  return (
    <div className="view-content">
      <div>
        <h1>Overview</h1>
        <p className="subtitle">Real-time visibility into Roxy's access decisions.</p>
      </div>
      ...
      <div className="section">
        ...
        <div className="alert-list">
          {alerts.length === 0 && <div className="empty-state">No denied requests.</div>}
          {alerts.map((log) => (
            <div key={log._id} className="alert-card" onClick={() => onSelect(log)}>
              <AlertTriangleIcon size={17} />
              <div className="alert-body">
                <div className="alert-title-row">
                  <span className="alert-mcp">{log.mcpName ?? 'Unknown MCP'}</span>
                  <span className="alert-status">Denied</span>
                </div>
          ...
          {activity.map((log) => {
            ...
            return (
              <div key={log._id} className="activity-row" onClick={() => onSelect(log)}>
                ...
                <span className="activity-mcp">{log.mcpName ?? 'Unknown MCP'}</span>
